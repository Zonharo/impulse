...
const DEFAULT_KERNEL_ARGS: &str = "console=ttyAMA0 earlycon=pl011,mmio32,0x09000000 root=LABEL=impulse-root rw";
...
fn choose_config() -> Option<PathBuf> {
    let output = Command::new("/usr/bin/osascript")
        .arg("-e")
        .arg("POSIX path of (choose file with prompt \"Choose a Impulse configuration\" of type {\"impulse\"})")
        .output()
        ...
        let base_image = config
            ...
            .unwrap_or_else(|| {
                resource_path(
                    ...
                    "build/guest/impulse-debian.raw.zst",
                )
            });
