...
pub fn install_main_menu() {
    ...
    unsafe {
        install_action(sel!(impulseToggleStatistics:), toggle_statistics_action);
        install_action(sel!(impulseShowAbout:), show_about_action);

        let app: *mut AnyObject = msg_send![class!(NSApplication), sharedApplication];
        let main_menu = new_menu("");

        let app_menu = new_menu("Impulse");
        add_item(
            app_menu,
            new_menu_item("About Impulse", Some(sel!(impulseShowAbout:)), "", 0, app),
        );
        add_separator(app_menu);
        add_item(
            app_menu,
            new_menu_item("Quit Impulse", Some(sel!(terminate:)), "q", COMMAND, app),
        );
        add_submenu(main_menu, "Impulse", app_menu);

        let view_menu = new_menu("View");
        let _: () = msg_send![view_menu, setAutoenablesItems: false];
        let statistics_item = new_menu_item(
            "Show Statistics",
            Some(sel!(impulseToggleStatistics:)),
            "s",
            COMMAND | SHIFT,
            app,
        );
...
pub fn show_about() {
    const DESCRIPTION: &str =
        "A lightweight virtual machine for Apple Silicon Macs.\nHardware-accelerated graphics, FEX, Steam and Proton on Debian.";
