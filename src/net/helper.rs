...
fn launch_watchdog(helper_pid: u32) -> io::Result<Child> {
    Command::new("/bin/sh")
        .args(["-c", WATCHDOG_SCRIPT, "impulse-vmnet-watchdog"])
...
fn helper_path() -> io::Result<PathBuf> {
    ...
    let contents = executable
        ...
        .ok_or_else(|| io::Error::other("cannot locate Impulse.app Contents directory"))?;
...
fn temporary_directory() -> io::Result<PathBuf> {
    let nonce = SystemTime::now()
        ...
        .unwrap_or_default()
        .as_nanos();
    let directory = Path::new("/tmp").join(format!("impulse-vmnet-{}-{nonce}", std::process::id()));
    DirBuilder::new()
        .mode(0o700)
        .create(&directory)
