build_impulse() {
  ...
  install -m 755 "$cargo_target_dir/release/impulse" "$BINARY"
...
build_icon() {
  ...
  local source="$runtime/Impulse.icon"
...
assemble_app() {
  ...
  install -m 755 "$BINARY" "$contents/MacOS/impulse"
...
<dict>
  ...
  <key>CFBundleIdentifier</key><string>dev.impulse.angle.$stem</string>
...
fix_rpaths() {
  ...
  local executable="$APP_BUNDLE/Contents/MacOS/impulse"
  local dylib index=0
...
verify_app() {
  local contents="$APP_BUNDLE/Contents"
  local executable="$contents/MacOS/impulse"
  ...
  [ "$(plutil -extract CFBundleExecutable raw -o - "$contents/Info.plist")" = impulse ] \
    || die "Info.plist does not reference impulse"
  [ "$(plutil -extract CFBundleIconName raw -o - "$contents/Info.plist")" = Impulse ] \
    || die "Info.plist does not reference the Impulse asset icon"
